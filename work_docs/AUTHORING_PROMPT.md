...
If you already know the exact code a task should produce, put it in exact_code and let description stay a short "why." Do NOT paraphrase known code into description and hope the executing model reconstructs it — small/medium local models are reliable at transcribing/reindenting code they're handed verbatim, but unreliable at inventing code from a description, especially one that quotes a signature in backticks (that reads as literal output to copy, not a spec to implement, and the model will often just echo the backtick text instead of writing a real implementation). Only omit exact_code when the exact code genuinely isn't decided yet and you want the executing model to work it out from description.
